      ...
      <nav className="container mx-auto px-4 py-3">
        <div className="flex justify-between items-center">
          ...
          <div className="hidden md:flex items-center space-x-6">
            <Link
              href="/"
              className={`text-gray-600 dark:text-gray-300 hover:text-blue-600 dark:hover:text-blue-400 transition-colors ${pathname === '/' ? 'font-semibold text-blue-600 dark:text-blue-400' : ''
                }`}
            >
              Home
            </Link>
            ...
            {isAuthenticated ? (
              <>
                {user?.is_staff && (
                  <Link
                    href="/admin"
                    className={`flex items-center px-3 py-1.5 rounded-lg transition-colors ${pathname.startsWith('/admin')
        ...
        {isMenuOpen && (
          <div className="absolute left-0 right-0 mt-2 px-4 pt-2 pb-4 bg-white dark:bg-gray-800 border-t dark:border-gray-700 md:hidden">
            <div className="space-y-3">
              <Link
                href="/"
                className={`block px-3 py-2 rounded-lg transition-colors ${pathname === '/'
              ...
              {isAuthenticated ? (
                <>
                  {user?.is_staff && (
                    <Link
                      href="/admin"
                      className={`flex items-center px-3 py-2 rounded-lg transition-colors ${pathname.startsWith('/admin')
                        ? 'bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400'
                        : 'text-gray-600 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700'
                        }`}
                    >
                  ...
                  <Link
                    href={`/profile/${user.username}`}
                    className={`flex items-center px-3 py-2 rounded-lg transition-colors ${pathname.startsWith('/profile')
                      ? 'bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400'
                      : 'text-gray-600 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700'
                      }`}
                  >
                    <div className="relative w-8 h-8 rounded-full overflow-hidden ring-2 ring-white dark:ring-gray-700 mr-2">
                      {user.avatar ? (
                        <Image
                          src={getFullAvatarUrl(user.avatar)}
                          alt={user.username}
