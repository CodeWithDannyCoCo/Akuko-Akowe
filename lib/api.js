...
async function fetchAPI(endpoint, options = {}) {
  ...
  // List of endpoints that don't require authentication for GET requests
  ...
  // Check if this is a public endpoint AND it's a GET request
  const isPublicEndpoint =
    publicEndpoints.some((pattern) =>
...
export const api = {
  ...
  getPersonalizedPosts: async (page = 1, limit = 10) => {
    const data = await fetchAPI(`/feed/?page=${page}&limit=${limit}`);
    return Array.isArray(data)
      ? {
          posts: data.sort(
            (a, b) => new Date(b.created_at) - new Date(a.created_at)
  ...
  getComments: (postId, page = 1, limit = 20) =>
    fetchAPI(`/comments/?post=${postId}&page=${page}&limit=${limit}`),

  getReplies: (commentId) => fetchAPI(`/comments/${commentId}/replies/`),
  ...
  unlikeComment: async (commentId) => {
    return await fetchAPI(`/comments/${commentId}/unlike/`, {
      method: "POST",
    });
  },
  ...
  getFollowers: (username) => fetchAPI(`/users/${username}/followers/`),

  getFollowing: (username) => fetchAPI(`/users/${username}/following/`),
  ...
  getDirectMessages: (username) => fetchAPI(`/messages/${username}/`),

  sendDirectMessage: (username, message) =>
    fetchAPI(`/messages/${username}/`, {
      method: "POST",
  ...
  getReactionTypes: async () => {
    const response = await fetch(`${API_URL}/api/reactions/types/`, {
      headers: {
        "Content-Type": "application/json",
  ...
  getCommentReactions: async (commentId) => {
    const response = await fetch(
      `${API_URL}/api/comments/${commentId}/reactions/`,
      {
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${localStorage.getItem("accessToken")}`,
  ...
  addCommentReaction: async (commentId, reactionTypeId) => {
    const response = await fetch(
  ...
  removeCommentReaction: async (commentId, reactionTypeId) => {
    const response = await fetch(
      `${API_URL}/api/comments/${commentId}/reactions/${reactionTypeId}/`,
      {
        method: "DELETE",
        headers: {
          Authorization: `Bearer ${localStorage.getItem("accessToken")}`,
